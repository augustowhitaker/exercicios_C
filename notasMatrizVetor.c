...
// Crie um programa que pede que o usuário digite 4 notas de 0 a 10 (tipo float) de 4 alunos e os valores deverão ser armazenados em uma matriz 
//bidimensional. Faça a média das notas de cada aluno em armazene as 4 médias em um vetor. Por ultimo  coloque na tela do usuário uma mensagem 
...
int main(void){
    ...
    for(int aluno = 0; aluno < numeroDeAlunos; aluno++){
        ...
        //  resetando a variavel media,pois senao irá ser somado a nota do aluno 1 com o do aluno 2...,para isso não ocorrer e calcular aluno por aluno
